...
この文書はプロジェクトプロジェクトの**全体スキルシステムを理解するためのハブ(Hub)**です。5分以内にスキルシステムの構造を把握し、必要なスキルを素早く見つけられるよう設計されています。
...
│ ┌─────────────┐ │ │ ┌─────────────┐ │ │ ┌─────────────────────┐ │
│ │architect    │ │ │ │implementer │ │ │ │pre-quality-gate     │ │
...
| スキル                                                   | 役割                       | ESP | トリガー                  |
| -------------------------------------------------------- | -------------------------- | :-: | ------------------------- |
| **[pre-quality-gate](../pre-quality-gate/SKILL.md)**                 | ビルド→Lint→テストサイクル | ✅  | "QA", "テスト実行"        |
...
├─ バグ修正 ───────────────→ bug-fix
│
├─ 品質検証 ───────────────┬→ pre-quality-gate (ビルド/Lint/テスト)
...
| **完璧な品質保証**         | persistent-mode + pre-quality-gate | 通過まで自動反復            |
| **デプロイ前検証**         | security-scan → pre-quality-gate   | セキュリティ + 品質同時検証 |
...
| pre-quality-gate   |      v3.1      | 検証対象、目標タイプ             |
...
| npm run build   | 生成まで | `src/**/*.ts`, `package.json` 変更 |
