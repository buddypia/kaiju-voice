...
name: turbo-mode
description: |
  Hackathon Project プロジェクトの **実際の並列実行エンジン**。
  oh-my-claudecode の ultrawork + ecomode + Crystal の TaskQueue コンセプトを統合。
...
| pubspec.yaml 読取          | **haiku**  | 単純パース                     |
| Provider/Service 検索      | **haiku**  | パターンマッチング             |
| DB スキーマ確認            | **haiku**  | メタデータ照会                 |
| Freezed Model 生成         | **sonnet** | テンプレート基準               |
| Riverpod ViewModel         | **sonnet** | パターン理解必要               |
| Widget 実装                | **sonnet** | Flutter パターン               |
...
| ---------------- | :------: | -------------------- | -------------------------------- |
| flutter analyze  |   30分   | lib/\*_/_.dart 変更  | `analyze_${timestamp}`           |
| flutter test     |   30分   | test/\*_/_.dart 変更 | `test_${timestamp}`              |
...
| ID  | 作業                            | タイプ | 予想ティア |
| --- | ------------------------------- | ------ | ---------- |
| T1  | lib/features/で類似パターン検索 | Read   | Haiku      |
| T2  | DB スキーマで関連テーブル確認   | Read   | Haiku      |
| T3  | 既存テストパターン調査          | Read   | Haiku      |
| T4  | CONTEXT.json 生成               | Write  | Sonnet     |
| T5  | SPEC.md 生成                    | Write  | Sonnet     |
| T6  | Model クラス実装                | Write  | Sonnet     |
| T7  | Repository 実装                 | Write  | Sonnet     |
| T8  | ViewModel 実装                  | Write  | Sonnet     |
| T9  | Widget 実装                     | Write  | Sonnet     |
...
| **同じファイル → 順次** | 同一ファイル修正は順次       | pubspec.yaml 修正は直列化               |
| **レイヤー依存性**      | Domain → Data → Presentation | Model → Repository → ViewModel → Widget |
...
| T9  | Widget 実装    | **sonnet** | general-purpose |
...
| pubspec.yaml 修正 | 依存性衝突危険   | 順次処理                |
| 同じファイル修正  | 内容衝突         | 1ファイル = 1ワーカー   |
| レイヤー依存性    | import 順序      | ViewModel → Widget 順次 |
...
| 日付       | バージョン | 変更内容                                                                                                                                             |
| ---------- | ---------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-02-01 | v1.0       | 新規生成 - ultrawork + ecomode 統合、Flutter 特化                                                                                                    |
